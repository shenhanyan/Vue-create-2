/**
 對axios进行二次封装(axios本身就是对原生ajax(XHR)的封装)   面试必说
    1，配置通用的基础路径和超时
    2，显示请求进度条
 ...
 */
 ...
 /* 1，配置通用的基础路径和超时 */
 // 创建一个新axios(Axios的功能上的实例)函数：功能与axios类似(可作为函数和对象的发送请求)
 const instance = axios.create({

   // baseURL:'http://182.92.128.115/api',// 前提是后台允许ajax跨越
 ...
 instance.interceptors.response.use(
     response => {
         console.log('请求拦截器成功回调执行')
         /** 2.2 请求成功结束  隐藏进度条 */
         NProgress.done()
         /** 3. 成功返回的数据不再是response，而是直接是响应体数据 response.data */
         // return response
